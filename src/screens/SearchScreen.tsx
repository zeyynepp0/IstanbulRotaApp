    ...
    <SafeAreaView className="flex-1 bg-background">
      <ScrollView className="flex-1">
        ...
        <View className="px-5 pt-5 pb-8 bg-primary rounded-b-3xl">
          <Text className="text-3xl font-bold text-white mb-2">İstanbul Rota Planlayıcı</Text>
        ...
        <View className="px-5 -mt-5">
          ...
          {/* Swap Button */}
          <TouchableOpacity 
            className={`self-center w-11 h-11 rounded-full bg-white justify-center items-center my-2 shadow-sm shadow-black/10 elevation-3 ${(!origin && !destination) ? 'opacity-50' : ''}`}
        ...
        {/* Search Button */}
        <TouchableOpacity
          className={`mx-5 mt-6 bg-primary py-4 rounded-xl items-center shadow-lg shadow-blue-500/30 elevation-4 ${(!origin || !destination) ? 'bg-gray-400 shadow-none' : ''}`}
        ...
        <View className="mx-5 mt-8 mb-5 p-5 bg-white rounded-2xl shadow-sm shadow-black/5 elevation-2">
          <Text className="text-lg font-semibold text-text-main mb-4">{ICONS.bulb} İpuçları</Text>
